...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['csrf_token'])) {
    try {
        ...
        $query = "INSERT INTO activity_logs (user_id, action, description, ip_address, related_id) 
                  VALUES (:admin_id, 'approve_client', :description, :ip_address, :client_id)";
        $description = "Aprobó el registro del cliente: " . $client['business_name'];
        $stmt = $db->prepare($query);
        ...
        $mailer = new Mailer();
        $mailer->sendAccountActivationEmail($client['email'], $client['business_name']);
        
        $db->commit();
        
        $_SESSION['success'] = "Cliente aprobado exitosamente";
